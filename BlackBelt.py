...
class AplicacionClubPro:
    ...
    def ventana_categorias(self):
        ...
        def agregar():
            ...
            if nombre:
                ...
                try:
                    ...
                    conn.commit(); ent_cat.delete(0, tk.END); cargar()
                except Exception as e:
                    conn.rollback()
        ...
        def cargar():
            ...
            for r in cur.fetchall(): lista_cat.insert(tk.END, f"  {r[0]}")
            liberar_db(conn)
        ...
        def eliminar():
            ...
            if messagebox.askyesno("Confirmar", f"¿Eliminar «{sel}»?"):
                ...
                conn.commit(); liberar_db(conn); cargar()
    ...
    def ventana_busqueda_individual(self):
        ...
        def buscar(event=None):
            ...
            if conn is None:
                ...
                return
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM miembros WHERE cedula=%s", (ent.get().strip(),))
    ...
    def cargar_datos_tabla(self):
        ...
        conn = conectar_db()
        if conn is None:
            return  # tabla queda vacía, no crashea — la ventana ya está construida
        cursor = conn.cursor()
        cursor.execute("""SELECT id, nombres, apellidos, cedula, categoria,
            ciudad_nacimiento, fecha_nacimiento, telefono, direccion, correo, fecha_ingreso
            FROM miembros""")
        for i, f in enumerate(cursor.fetchall()):
            tag = "par" if i % 2 == 0 else "impar"
            # Formatear fechas (PG devuelve date objects)
            f = list(f)
            f[6]  = _fmt_fecha(f[6])   # fecha_nacimiento
            f[10] = _fmt_fecha(f[10])  # fecha_ingreso
            self.tabla.insert("", "end", values=tuple(f), tags=(tag,))
    ...
    def ver_desde_tabla(self):
        ...
        conn = conectar_db()
        if conn is None:
            messagebox.showerror("Sin conexión", "Sin conexión a PostgreSQL.")
            return
        cursor = conn.cursor()
    ...
    def ventana_eventos(self):
        ...
        def cargar_eventos():
            ...
            for i, ev in enumerate(cur.fetchall()):
                cur.execute("SELECT COUNT(*) FROM evento_participantes WHERE evento_id=%s", (ev[0],))
                cant = cur.fetchone()[0]
                tag = "par" if i % 2 == 0 else "impar"
                tabla_ev.insert("", "end",
        ...
        def eliminar_sel():
            ...
            if messagebox.askyesno("Confirmar", f"Eliminar el evento \"{enomb}\"?\nSe eliminaran sus participantes."):
                ...
                conn.commit(); liberar_db(conn); cargar_eventos()
    ...
    def ventana_crear_evento(self, callback_reload, evento_id=None):
        ...
        try:
            ...
            if conn:
                cur = conn.cursor()
                cur.execute("SELECT DISTINCT categoria FROM miembros WHERE categoria IS NOT NULL AND categoria != '' ORDER BY categoria")
                opciones_cat = ["— Todas —"] + [r[0] for r in cur.fetchall()]
                cur.execute("SELECT DISTINCT genero FROM miembros WHERE genero IS NOT NULL AND genero != '' ORDER BY genero")
                opciones_gen = ["— Todos —"] + [r[0] for r in cur.fetchall()]
                cur.execute("SELECT DISTINCT ciudad_nacimiento FROM miembros WHERE ciudad_nacimiento IS NOT NULL AND ciudad_nacimiento != '' ORDER BY ciudad_nacimiento")
                opciones_ciu = ["— Todas —"] + [r[0] for r in cur.fetchall()]
                liberar_db(conn)
            else:
                opciones_cat = ["— Todas —"]
                opciones_gen = ["— Todos —"]
        ...
        def aplicar_filtros():
            ...
            conn = conectar_db()
            if conn is None:
                return  # tabla queda vacía, UI ya construida
            cur = conn.cursor()
            cur.execute("""SELECT id, nombres, apellidos, cedula, categoria, genero, ciudad_nacimiento
            ...
            for m in cur.fetchall():
                ...
                # Búsqueda por nombre con soporte de tildes
                if f_busq:
